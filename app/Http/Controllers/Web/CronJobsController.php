...
class CronJobsController extends Controller
{
    ...
    public function sendMeetingPackageReminders($request)
    {
        $mixin = (new \App\Mixins\MeetingPackages\MeetingPackageNotificationsMixins());
        $mixin->sendSessionsReminders();

        return response()->json([
            'status' => 200,
            'message' => "Job Started for send Meeting Packages (Sessions) Reminders"
        ]);
    }


    /**
     * @description Using this job, we can renew plans that are about to expire.
     * @description This job should be run once a day.
     * */
    public function renewSubscriptions()
    {
        $mixin = (new \App\Mixins\RegistrationPackage\AutoRenewSubscriptionMixins());
        $mixin->renewSubscriptionsPlans();

        return response()->json([
            'status' => "Ok!",
            'message' => "Renewal of plans that expire today."
        ]);
    }

    /**
     * @description Use this job to send notifications before plans expire.
     * @description The frequency of execution of this job depends on the hour entered in the settings (/settings/general/Reminders). For example, if 5 is entered there, then this job should be executed every 5 hours.
     * */
    public function reminderBeforeExpirationSubscribes()
    {
        $mixin = (new \App\Mixins\RegistrationPackage\AutoRenewSubscriptionMixins());
        $mixin->reminderBeforeExpiration();

        return response()->json([
            'status' => "Ok!",
            'message' => "send notifications before plans expire"
        ]);
    }
    ...
    public function sendAttendanceNotifications(Request $request)
    {
        $attendanceNotifications = (new \App\Mixins\Attendances\AttendanceNotifications());
        $attendanceNotifications->sendNotifToAttendances();


        return response()->json([
            'status' => 200,
            'message' => "Job Started for send Attendance Notifications"
        ]);
    }
    ...
    public function sendEventsReminders($request)
    {
        $mixin = (new \App\Mixins\Events\EventNotificationsMixins());
        $mixin->sendEventsReminders();

        return response()->json([
            'status' => 200,
            'message' => "Job Started for send Events (Events) Reminders"
        ]);
    }
