...
class CreateUserBanksTable extends Migration
{
    ...
    public function up()
    {
        ...
        if (!Schema::hasTable('user_bank_translations')) {
        ...
        if (!Schema::hasTable('user_bank_specifications')) {
        ...
        if (!Schema::hasTable('user_bank_specification_translations')) {
        ...
        Schema::table('users', function (Blueprint $table) {
            ...
            if (Schema::hasColumn('users', 'account_type')) {
                $dropColumns[] = 'DROP COLUMN `account_type`';
            }

            if (Schema::hasColumn('users', 'iban')) {
                $dropColumns[] = 'DROP COLUMN `iban`';
            }

            if (Schema::hasColumn('users', 'account_id')) {
                $dropColumns[] = 'DROP COLUMN `account_id`';
            }

            if (!empty($dropColumns)) {
                \Illuminate\Support\Facades\DB::statement("ALTER TABLE `users` " . implode(',', $dropColumns) . ";");
            }
        });

        if (!Schema::hasTable('user_selected_banks')) {
        ...
        if (!Schema::hasTable('user_selected_bank_specifications')) {
